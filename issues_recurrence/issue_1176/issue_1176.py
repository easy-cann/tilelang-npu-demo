...
def matmul_add(M, N, K, block_M, block_N, block_K, dtype="float32", accum_dtype="float"):
    m_num = M // block_M
    n_num = N // block_N
    ...
    def main(
        ...
        with T.Kernel(m_num * n_num, threads=2, is_npu=True) as (cid):
            ...
            d_ub_float = T.alloc_shared((block_M, block_N), accum_dtype)
            c_ub_float = T.alloc_shared((block_M, block_N), accum_dtype)
            ...
            T.copy(c_ub, c_ub_float)
            T.copy(d_ub, d_ub_float)

            T.tile.add(c_ub_float, c_ub_float, d_ub_float)

            T.copy(c_ub_float, c_ub)
...
a = torch.randn(M, K).float32().npu()
b = torch.randn(K, N).float32().npu()
d = torch.randn(M, N).float32().npu()
