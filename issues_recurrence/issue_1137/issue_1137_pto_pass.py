...
def get_topk_gate_kernel(num_experts: int, num_topk: int):
    ...
    num_threads = 32
    # Ensure number of experts is aligned to 32 for NPU vector instruction alignment
    num_aligned_experts = (num_experts + num_threads - 1) // num_threads * num_threads

    VEC_NUM = 2
    num_blocks = (num_tokens + VEC_NUM - 1) // VEC_NUM
    ...
    def topk_gate_kernel(
        ...
        with T.Kernel(num_blocks, is_npu=True) as (cid, vid):
            token_idx = T.min(cid * VEC_NUM + vid, num_tokens - 1)

            scores_ub = T.alloc_ub((num_aligned_experts,), "float32")
            topk_dst_ub = T.alloc_ub((2 * num_topk,), "float32")
            topk_index_f32 = T.alloc_ub((num_topk,), "float32")
            topk_idx_out_ub = T.alloc_ub((num_topk,), "int32")

            T.copy(scores[token_idx, 0:num_experts], scores_ub[0:num_experts])
            for i in range(num_aligned_experts - num_experts):
                scores_ub[num_experts + i] = -1e10

            T.tile.topk(topk_dst_ub, scores_ub, num_topk, num_aligned_experts)

            T.tile.gather_mask(topk_index_f32, topk_dst_ub, "P1010")
            T.tile.cast(topk_idx_out_ub, topk_index_f32, "CAST_ROUND", num_topk)
            T.copy(topk_idx_out_ub[0:num_topk], topk_idx[token_idx, 0:num_topk])
...
def topk_gate(scores: torch.Tensor, num_topk: int) -> torch.Tensor:
    ...
    topk_idx = torch.empty((num_tokens, num_topk), dtype=torch.int32, device=scores.device)  # int32 instead of int64
    if num_tokens == 0:
        return topk_idx
